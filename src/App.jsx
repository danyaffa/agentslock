...
import { auth, db, firebaseError, signUp, logIn, logOut, signInWithGoogle, onAuth, loadUserData, saveUserData, saveSubscription, loadSubscription } from "./firebase.js";
...
function useAuth() {
  ...
  const doGoogleLogin = async () => {
    try { await signInWithGoogle(); return { ok: true }; }
    catch (e) { return { ok: false, err: e.message.replace("Firebase: ", "").replace(/\(auth\/.*\)/, "").trim() }; }
  };
      ...
      <div style={{ width: 400, padding: 40 }}>
        ...
        <Card>
          ...
          <button onClick={async () => {
            ...
            width: "100%", padding: "12px", border: `1px solid ${C.border}`, borderRadius: 8,
            background: C.bg, color: C.bright, cursor: busy ? "not-allowed" : "pointer",
            fontFamily: "inherit", fontSize: 13, fontWeight: 600, display: "flex",
            alignItems: "center", justifyContent: "center", gap: 10, opacity: busy ? 0.6 : 1,
...
export default function App() {
  ...
  if (!user) return <AuthScreen onLogin={login} onSignup={signup} onGoogleLogin={googleLogin} />;
